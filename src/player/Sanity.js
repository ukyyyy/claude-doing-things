...
export class SanitySystem {
  ...
  update(dt, { inSafeZone, flashlightOn, enemyDistance, enemyChasing }) {
    const drainMult = (DIFFICULTY_PRESETS[this.settings?.difficulty] || DIFFICULTY_PRESETS.normal).sanityDrain;
    let delta = 0;
    if (inSafeZone) {
      delta += REGEN_SAFE;
    } else {
      if (!flashlightOn) delta -= DRAIN_DARK * drainMult;
      if (enemyChasing) delta -= DRAIN_CHASE * drainMult;
      else if (enemyDistance < 9) delta -= DRAIN_NEAR * drainMult * (1 - enemyDistance / 9);
    }
